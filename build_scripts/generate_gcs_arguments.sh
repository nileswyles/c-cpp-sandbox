...
# ! IMPORTANT -
# 		Make sure to run `$WYLESLIBS_WORKSPACE_ROOT_DIR/build_scripts/build_google.sh` before this script to make sure google deps are initialized.
...
if [ -z $GOOGLE_CLOUD_VCPKG_INSTALL_DIR ]; then
    GOOGLE_CLOUD_VCPKG_INSTALL_DIR="$WYLESLIBS_WORKSPACE_ROOT_DIR/google_cloud_cpp_install/x64-linux"
fi
