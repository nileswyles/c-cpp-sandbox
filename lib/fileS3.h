...
class S3FileManager: public FileManager {
    public:
        ...
        SharedArray<uint8_t> read(std::string path, size_t offset = 0) override final;
        SharedArray<uint8_t> read(std::string path, size_t offset = 0, size_t size) override final;
