...
// TODO: portability? - okay, so confirmed microsoft's visual-c supports this functionality and I think the time module works fine, just need to test.
//          what other interesting things can we do with macros?

// NOTE: ## removes trailing comma when args is empty.

// ! IMPORTANT - Required for the ecal and string format tests because they test the functionality this toggles.
//                  This can probably be removed once those tests are stable enough.
...
#define loggerPrintf(min, fmt, ...) \
    if (LOGGER_LEVEL >= min && LOGGER_MODULE_ENABLED) {\
        FILE * file = logger_config.stderr;\
        if (LOGGER_LEVEL >= LOGGER_TEST) {\
            file = logger_config.stdout;\
        }\
