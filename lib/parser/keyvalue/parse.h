...
extern std::unordered_map<std::string, std::string> parse(IOStream * reader, char key_delim, char delim);

// LOL
static std::unordered_map<std::string, std::string> parse(std::string s, char delim) {
    return parse(s, '=', delim);
}
static std::unordered_map<std::string, std::string> parse(IOStream * reader, char delim) {
