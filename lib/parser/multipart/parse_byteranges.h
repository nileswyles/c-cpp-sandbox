...
static void parse(ByteEStream * r, MultipartFile& file) {
    while (1) {
        SharedArray<uint8_t> potential_boundary = r->read<SharedArray<uint8_t>>("\n"); // read and consume boundary string
        if (potential_boundary.at(0) == '-' && potential_boundary.at(1) == '-') {
            ...
            r->read<SharedArray<uint8_t>>(" "); // read and consume Content-Range: |
            r->read<SharedArray<uint8_t>>(" "); // read and consume bytes |
            ...
            // [0,0] = 1, [1,0] = 0, [0,1] = 2
            //            [2,0] = -1
            SharedArray<uint8_t> data = r->read<SharedArray<uint8_t>>(max - min + 1);
